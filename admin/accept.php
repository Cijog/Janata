...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['aid'])) {
    ...
    $updateStatusQuery = "UPDATE topup SET status = 2 WHERE order_id = ?";
    $stmtUpdateStatus = $conn->prepare($updateStatusQuery);
    $stmtUpdateStatus->bind_param("i", $order_id);

    if ($stmtUpdateStatus->execute()) {
        $_SESSION['msg'] = "Order accepted successfully!";
    } else {
        $_SESSION['msg'] = "Error accepting order: " . $stmtUpdateStatus->error;
    }
